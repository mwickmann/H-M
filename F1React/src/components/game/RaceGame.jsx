...
const RaceGame = () => {
  ...
  // starter racet , setter posisjon på alle cars

  const startRace = () => {
    setTimeout(() => {
      moveCar(1100, setPositionCar1, 1); 
    }, 0); 
  ...
  useEffect(() => {
   ...
   const resetRace = () => {
    // Tilbakestiller posisjonene og tilbakestillingsvariabler
    setPositionCar1(0);
    setPositionCar2(0);
    setPositionCar3(0);
    setRaceFinished(false);
    setWinnerMessage('');
    setFinishedCarsCount(0);
  };
return (
    <div className='row g-5 m-5'> 
    <section className=''>
      <h2 className='text-center race-title mb-5'>Can Rolando beat the best drivers in the world? </h2>
      <div id='game-btns'>
      <button onClick={startRace} className='btn btn-outline-dark justify-content-center d-flex align-items-center'>READY, SET, GO!</button>
      <button onClick={resetRace} className='btn btn-outline-dark justify-content-center d-flex align-items-cente'>RESET</button>
      </div>


      <div className="race-container mb-5">
        <img
