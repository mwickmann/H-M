...
const DriversSearchTeam = () => {
  const { getByTeam } = useContext(DriversContext);
  ...
  const handleSearch = async () => {
    try {
      const results = await getByTeam(searchTerm);
      setSearchResults(results);
      setSearchError("");
    } catch (error) {
      setSearchResults([]);
      setSearchError("No drivers found for the specified team.");
    }
  };
  ...
  return (
    <section>
      <h3 className="search-txt">Search by Team</h3>
      <div>
        <input
          type="text"
          value={searchTerm}
        ...
        <button onClick={handleClear} className="search-txt">Clear</button>
      </div>
...
export default DriversSearchTeam;